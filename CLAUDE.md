...
- **Streak:** approval on or before the current plan's `end_date` sets `streak_month = old + 1`, else 1. Monthly tiers get `streak_discount_ron(streak)` off (15/30/40 RON); `price_paid_ron` / `discount_ron` are recorded on the plan.
