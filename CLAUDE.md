...
- **Plan required and not expired.** `sessions_used < sessions_total` AND `plan.end_date >= session date`. When the active plan cannot cover a booking and the member has a **queued** plan, the queued plan activates on the spot (`resolve_plan_for_booking`) and the booking draws from it; otherwise the booking is rejected (grace bookings were removed in Aug 2026; `bookings.is_grace` only guards refunds of legacy rows).
