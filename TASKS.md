...
- [x] **11.9 Live schema reconciliation.** The project had branch `claude/app-features-addition-50b8ef` (grace removed, streak discounts, `is_scheduled` renewals, `admin_grant_plan`, health consent) applied through the SQL editor. Migration `0023` merges that with the status model: no grace in the booking functions, streak/price restored in `approve_plan_request`, `admin_grant_plan` and `freeze_membership` status-aware, `is_scheduled` and `activate_due_scheduled_plans` dropped. Migration history repaired (0001–0018 marked applied). The branch's app code (streak UI, grant-plan form, health consent) is NOT in this branch yet.
