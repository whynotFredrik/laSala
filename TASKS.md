...
- [x] **11.10 Renewal merges sessions.** Migration `0027`: an on-time approval (≤ current `end_date`) activates the new plan immediately with the unused sessions added and validity extended from the old `end_date` + duration; the queued-plan mechanism (0022) is removed (`plans.status` = active | ended). Admin row shows the merge outcome; member texts updated.
- [x] **11.11 Admin notifications.** Every admin account gets an in-app notification + email for each new plan request and a daily digest of plans expiring in 7/3/1 days (`lib/notifications/admins.ts`; `ADMIN_NOTIFICATION_EMAIL` is now an optional extra inbox). Bell in the admin nav, inbox at `/admin/notifications`, shared list component with the member inbox.
- [x] **11.12 Streak monthly-only.** Migration `0028`: the streak continues only monthly → monthly on time; promotions carry `streak_month = 1`, get no discount, and the first monthly plan after one starts at 1. `admin_grant_plan` ignores the streak input for promo tiers. Member/admin pages show "promoție, fără streak" instead of a streak month.
- [x] **11.13 Admin "Expirări" page.** `/admin/expiring`: active plans whose `end_date` falls in the studio week (Mon–Sun), grouped by day, with plan, sessions left, streak and a "cerere în așteptare" badge; `?week=±n` moves between weeks. Linked from the admin nav.
