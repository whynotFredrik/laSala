...
- **Plan required and not expired.** `sessions_used < sessions_total` AND `plan.end_date >= session date`. When the active plan cannot cover a booking it is rejected (grace bookings were removed in Aug 2026; `bookings.is_grace` only guards refunds of legacy rows).
- **Renewal merges:** a member with an active plan may request another one at any time. If the admin approves no later than the current plan's `end_date` (on time), the new plan starts immediately, its total includes the unused sessions of the current plan, and it runs until the current `end_date` + the tier's duration (`approve_plan_request`, migration 0027). Otherwise the plan starts fresh on the chosen date. `plans.status` is `active` | `ended`.
- **Streak:** monthly tiers only. Approval on or before the current plan's `end_date`, when both the current and the new tier are `monthly`, sets `streak_month = old + 1`; anything else (late, promo bought, or coming off a promo) is 1. Promotions never hold a streak. Monthly tiers get `streak_discount_ron(streak)` off (15/30/40 RON); `price_paid_ron` / `discount_ron` are recorded on the plan (migration 0028, `lib/plans/streak.ts`).
